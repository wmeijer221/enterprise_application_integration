<script lang="ts">
	...
	onMount(async () => {
		const actorDetailResponse = await fetch(`http://localhost:8000/actor?id=${id}`);

		if (actorDetailResponse.ok) {
			actorDetails = await actorDetailResponse.json();
		}

		const actorSentimentResponse = await fetch(
			`http://localhost:8000/actors/sentiment?actor_id=${id}`
		);

		if (actorSentimentResponse.ok) {
			averageSentiment = await actorSentimentResponse.json();
		}

		const sentimentMessagesResponse = await fetch(
			`http://localhost:8000/actors/sentiment/examples?actor_id=${id}`
		);
		...
		ws = new WebSocket('ws://localhost:8082/actors/sentiment?actor_id=' + id);
...
<div class="p-16 bg-gradient-to-b from-neutral-200 to-neutral-300 text-neutral-900">
	<h1 class="pb-2">{actorDetails.name}</h1>
	<!-- <div>
		{sentimentCount}
